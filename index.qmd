...
* **Coding Style:** Some key components of Python mesa models.
...
* **Crashcourse:** Understand what key functionality Mesa has for agent-based modeling.
* **Dispersal:** Learn about the dispersal model and deconstruct it conceptually for translation.
* **Part I: Python implementation from scratch:** Learn how to begin translating a NetLogo model to Python Mesa.
...
* **Implementation from scratch Part II:** We continue to practice model translation.
* **Solving problems yourself:** Visualization,  agent behavior, and data plotting.
...
* **Recap:** Recap of previous materials.
* **Creative adaptation:** Create your own model variations.
* **Create measurements**. Design measures and use Mesa's data collector and native plotting.
...
* **Introduction to adaptive agents**: How to have your agents have memory and adapt.
* **Models:** Detailed understanding of El Farol model.
* **Introduction to Parameter sweeps**: Learn how to do parameter sweeps and report results.
...
* **Intro to agent-sets**: Introduction to agent-sets via the predator-prey model.
* **Analysis**: Python (time series) analysis and visualization of results.
...
This installation I have tested on the practical machines and on a windows PC. Feel free to deviate if you have different preferences for your python installation.
...
## Method 2
Download the environment.yml file here [environment.yml](./quarto_dependencies/mesa_env/environment.yml) and run the following command in your terminal:
...
## Basic models
* [Basic Grid, Continuous, Network Model Folder Python](./models/basic_python_models.rar)
* [Basic Grid, Continuous, Network Model Folder Netlogo](./models/basic_netlogo_models.rar)
...
* [Example custom visualizations folder](./models/examples_custom_visualizations.rar)
...
* [Download Full Functionality Overview Model Folder Python](./models/fullfunc_grid_python_model.rar)
...
* [Download Dispersal Model Folder Netlogo](./models/dispersal_netlogo.rar)
...
* [Download Dispersal Model Translation Part II Folder Python (incomplete)](./models/dispersal_python_partII.rar)

### Simple Dispersal Model After Practicum 3
* [Dispersal Model Simple Folder Python (partially completed)](./models/dispersal_python_afterpartII.rar)

### Python challenge
* [Dispersal Model with Worldmap Folder Python (incomplete)](./models/dispersal_python_model_incomplete.rar)
* [Dispersal Model with Worldmap Folder Python (complete)](./models/dispersal_python_model_complete.rar)
:::
...
::: {.callout-note .callout-download collapse="true"}
## Flocking model
* [Flocking Model Folder Python](./models/flocking_model_python.rar)
* [Flocking Model Folder Netlogo](./models/flocking_model_netlogo.rar)
:::
::: {.callout-note .callout-download collapse="true"}
## El Farol Model
* [El Farol Model Folder Python](./models/elfarol_model_python.rar)
  * [El Farol Solution Practicum 5](./models/elfarol_model_extended.rar)
* [El Farol Model Folder Netlogo](./models/elfarol_model_netlogo.rar)
:::
...
::: {.callout-note .callout-download collapse="true"}
## Wolf-Sheep Model
* [Wolf-Sheep Model Folder Python](./models/wolfsheep_model_python.rar)
* [Wolf-Sheep Model Folder Python (practicum 6 solution)](./models/wolfsheep_solution_python.rar)
* [Wolf-Sheep Folder Netlogo](./models/wolfsheep_model_netlogo.rar)
:::
...
### Exercise 1: what is a class {#ex-1a-1}

A class is a template. It says what an object knows (attributes) and what it can do (methods). You can make an instance of a class, and another separate instance, which then becomes a separate version of the template. `self` is the particular instance the method runs on, and it is initialized with certain attributes.

Run this: `a` and `b` are separate objects from one template.
...
**Your turn.** Fill in the two gaps so the sheep has an `energy` attribute and an `eat` method that adds 5. Running it should print 15.
...
## Hint
Attributes are stored on `self`, and the name you store under is the name you read back later. To add in place, use `+=`.
:::

### Exercise 2: why super() {#ex-1a-2}

In your python mesa models you often see `class SimpleGridAgent(CellAgent)`, which means that your class inherits methods from `CellAgent`. The parent's `__init__` does bookkeeping, such that it registers the agent with the model, handing out a unique id, or for example attaching a random generator. Writing your own `__init__` would overwrite the parent class' `__init__`, were it not for `super().__init__(...)`, which runs the parent's version first.
...
### Exercise 3: inherit and extend {#ex-1a-3}

Make `Wolf` inherit from `Animal`, reuse the parent for `energy`, and add only `hunger`. It should print `20 3`.
...
class Wolf(???):
    def __init__(self, energy, hunger):
        ???
        self.hunger = hunger

w = Wolf(20, 3)
print(w.energy, w.hunger)
```

::: {.callout-tip collapse="true"}
## Hint
The parent class goes in the brackets after the class name. The second gap needs to hand `energy` up to the parent, so it takes an argument.
:::
