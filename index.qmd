...
::::: {.callout-note .callout-download collapse="true"}
## For mac OS: Installation guide python using anaconda + mesa
For Mac users the above procedure does not work and you need to follower a slightly different procedure. After installing anaconde you need to run conda commands in your terminal instead (you can search for your "terminal" in the MAC OS search panel). And another key difference is that we do not explicitly set the version for mesa (and you need download extra dependencies such as networkx in addition). So MAC OS users please look below:
...
pip install jupyter notebook seaborn pandas numpy matplotlib networkx altair solara
