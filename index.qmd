...
We will not actually give you the python version of the to be translated model, because we are going to build it together on the basis of the netlogo model. 

<!--
For the final fully implemented one one we are going to give you a hint and you can check out a start here:
...
::: {.callout-note .callout-download collapse=false"}
## Slides Practicum 2b
<iframe src="slides_practicals/Practical2b.html" width="100%" height="500" frameborder="0" allowfullscreen></iframe>

[Open slides in new window](./slides_practicals/Practical2b.html)
...
Mesa's DataCollector is quite simple. The DataCollector is called at every step, and `batch_run` is a loop inside a loop (but with parralelization). You could of course code up such a thing by hand. Run this first.
...
class Model:
    def __init__(self, numagents=2, seed=None):
        self.random = random.Random(seed)
        self.xs = [self.random.randrange(10) for _ in range(numagents)]

    def step(self):
        self.xs = [(x + self.random.choice([-1, 1])) % 10 for x in self.xs]
...
### Exercise 1: a model reporter {#ex-2a-1}

In the fullfunc notebook, `maxdistance(model)` is a plain function that takes the model and returns one number. Mesa just calls it this reporter function for you.

Write `spread`, returning the distance between the leftmost and rightmost agent.
...
print(spread(Model(numagents=10, seed=1)))   # 8
```

::: {.callout-tip collapse="true" title="Hint"}
The widest gap in a list of numbers is the largest minus the smallest.
:::

### Exercise 2: collecting over time {#ex-2a-2}

A DataCollector calls the reporter every step and appends the result to a growing table. Do that by hand. You should get 21 numbers: one from setup, plus one per step.

```{pyodide}
#| autorun: false
def spread(model):
    return max(model.xs) - min(model.xs)

m = Model(numagents=10, seed=1)
history = [spread(m)]        # collect once at setup, as the notebook does
...
::: {.callout-tip collapse="true" title="Hint"}
The line in the loop is the simlar as the one above it, appending after each step instead.
:::

In Mesa calling the spread function via the DataCollector looks like this: `mesa.DataCollector(model_reporters={"spread": spread})` plus `self.datacollector.collect(self)` at the end of `step`.

### Exercise 3: a batch run is two loops {#ex-2a-3}

One run tells you little about your typical model dynamics. Different seeds will give different starting positions. `batch_run` runs every parameter setting several times. Handcoded it would look something like this: outer loop over parameters, inner loop over seeds. Fill in both. You want 15 rows of result data.
...
results = []
for numagents in [2, 5, 10]:
    for seed in range(???):
        m = Model(numagents=numagents, seed=seed)
        for _ in range(20):
            m.step()
        results.append({"numagents": numagents, 
            "seed": seed, "spread": ???})

print(len(results), results[:3])
```

Note in Mesa this would like look: `mesa.batch_run(Model, parameters={"numagents": (2, 5, 10)}, iterations=5, max_steps=20)`. Same three nested things, as keyword arguments.

::::

## Practicum 2b {#ex-2b .active}

The dispersal model has agents that do one thing: now and then they make a copy of themselves in a neighbouring cell. These excercises should help generate intutitions for how to implement it in mesa.

:::: {.panel-tabset}


### Exercise 1: hatching {#ex-2b-1}

Same shape as practicum 1b. `BaseAgent` stands in for mesa's `CellAgent`, and `super().__init__(model)` is what puts the new agent in `model.agents`. An agent hatches a copy of itself with probability `growth_rate`, every step. Fill in the gap.
...
class BaseAgent:
    def __init__(self, model):
        self.model = model
        model.agents.append(self)          # the registration, as in mesa

class DispersalAgent(BaseAgent):
    def __init__(self, model, growth_rate):
        super().__init__(model)            # so the new agent joins model.agents
        self.growth_rate = growth_rate

    def hatch(self):
        if self.model.random.random() < self.growth_rate:
            ???                            # make a new agent, just like me

class DispersalModel:
    def __init__(self, n=5, growth_rate=0.1, seed=1):
        self.random = random.Random(seed)
        self.agents = []
        for _ in range(n):
            DispersalAgent(self, growth_rate)

    def step(self):
        for agent in list(self.agents):    # a copy of the list, see the hint
            agent.hatch()

model = DispersalModel()
for t in range(1, 21):
    model.step()
    print(t, len(model.agents))
```

Note that the hatching agent never touches `model.agents` itself. It just makes an instance, and `super()` does the joining. Take the `super()` line out and the population stays at five forever, which is the silent bug you might get when failing to inherit from the parent calss.

Note also that `growth_rate` is per agent per step, so the population does not grow by a fixed amount but by a fixed proportion. Set it to 0.3 and look at the last number.

::: {.callout-tip collapse="true"}
## Hint
You can call an agent within the agent class. The new agent is created by calling the class constructor (the class constructor your in, which might be a bit confusing) with the appropriate arguments.
:::

### Exercise 2: where can I hatch into? {#ex-2b-2}

An agent cannot hatch just anywhere. The new one goes in a neighbouring cell that is free, and in the full model there is water where nobody can live. Fill in the filter.
...
occupied = {(3, 3), (3, 4), (4, 3)}     # cells that already hold an agent
water    = {(2, 2), (2, 3), (2, 4)}     # cells nobody can live in
x, y = 3, 3                             # me

neighbours = [(x + dx, y + dy) for dx in (-1, 0, 1) for dy in (-1, 0, 1)
              if (dx, dy) != (0, 0)]

free = [cell for cell in neighbours if ???]

print("neighbours:", sorted(neighbours))
print("free      :", sorted(free))
print("hatch into:", rng.choice(free))
```

You should get three free cells. In the real model these two lines are the same idea:

```python
empty_cells = [cell for cell in neighbours if len(cell.agents) == 0]
empty_cells = [cell for cell in empty_cells if binary_matrix[cell.coordinate[1], cell.coordinate[0]] == 1]
```

::: {.callout-tip collapse="true"}
## Hint
Two conditions joined with `and`, both of them a `not in`.
:::
...
- [Emergent phenomena from sorting algorithms (with Michael Levin)](https://www.youtube.com/watch?v=jr1sNYY2t9A)
...
- "Agnosiophobia in a virtual agent: behavioral and dynamical architecture in Lenia" (download [PDF](https://arxiv.org/pdf/2605.30708))
...
::: {.callout-note .callout-download collapse="true"}
## Recording Practicum 2a

[Recording link to Panopto](https://tilburguniversity.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=d8446f3f-0033-4b26-b90a-b4bf00912ed5)
```{=html}
<div style="position: relative; width: 100%; height: 0; padding-bottom: 56.25%;">
<iframe src="https://tilburguniversity.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=d8446f3f-0033-4b26-b90a-b4bf00912ed5&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=false&interactivity=all"
style="border: 1px solid #464646; position: absolute; top: 0; left: 0; width: 100%; height: 100%; box-sizing: border-box;"
allowfullscreen
allow="autoplay"
aria-label="Practical 1a MAS 2026/2027">
...
::: {.callout-note .callout-download collapse="true"}
## Recording Practicum 2b

[Recording link to Panopto](https://tilburguniversity.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=b9a34fab-caec-46f3-b027-b4c900b1a64c)
```{=html}
<div style="position: relative; width: 100%; height: 0; padding-bottom: 56.25%;">
<iframe src="https://tilburguniversity.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=b9a34fab-caec-46f3-b027-b4c900b1a64c&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=false&interactivity=all"
style="border: 1px solid #464646; position: absolute; top: 0; left: 0; width: 100%; height: 100%; box-sizing: border-box;"
allowfullscreen
allow="autoplay"
aria-label="Practical 1a MAS 2026/2027">
