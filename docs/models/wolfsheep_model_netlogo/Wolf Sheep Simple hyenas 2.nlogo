...
to setup
  ...
  create-hyenas number-of-hyenas[
    setxy random-xcor random-ycor
    set color red
    set energy 100  ;; set the initial energy to 100
  ]
...
;; wolf/hyena procedure, wolves and hyena eat sheep
to eat-sheep
...
to eat-hyena
  if any? hyenas-here [
    ...
    set energy energy + energy-gain-from-hyena
  ]
end
...
to eat-wolf
  if any? wolves-here [
    let target one-of wolves-here
    ask target [
      die
    ]
    set energy energy + energy-gain-from-wolf
  ]
end
...
to my-update-plots
  ...
  set-current-plot-pen "hyenas"
  plot count hyenas * 10
